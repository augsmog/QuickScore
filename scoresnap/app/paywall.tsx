...
export default function PaywallScreen() {
  ...
  const handlePurchase = async (pkg: PurchasesPackage | null) => {
    if (!pkg) {
      Alert.alert(
        "Not Available",
        "Subscriptions are not yet configured. Please try again later."
      );
      return;
    }
    setLoading(true);
  ...
  const features = [
    {
      ...
      label: "Unlimited scorecard scans",
      desc: "AI-powered OCR for handwritten & printed scorecards",
    },
    {
      icon: <Trophy size={18} color={COLORS.accent} />,
      label: "All 25+ game modes",
      desc: "Wolf, Banker, Hammer, Vegas, Nines, and more",
    },
    {
      icon: <DollarSign size={18} color={COLORS.accent} />,
      label: "Settlement tracking",
      desc: "See who owes whom across all games",
    },
    {
      icon: <Zap size={18} color={COLORS.accent} />,
      label: "Share & export results",
      desc: "Send results to your group instantly",
    },
  ];

  // Use live pricing from RevenueCat if available
    ...
    >
      ...
      >
        ...
        <View style={{ alignItems: "center", marginTop: 8, marginBottom: 28 }}>
          ...
          <Text
            style={{ color: COLORS.text, fontWeight: "800", fontSize: 26 }}
          >
            ScoreSnap Pro
          </Text>
          <Text
        ...
        >
          ...
          >
            <Text
              style={{ color: COLORS.gold, fontWeight: "800", fontSize: 34 }}
            >
              {annualPrice}
            </Text>
